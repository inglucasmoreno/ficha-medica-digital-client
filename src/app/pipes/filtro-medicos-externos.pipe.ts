...
@Pipe({
  name: 'filtroMedicosExternos'
})
export class FiltroMedicosExternosPipe implements PipeTransform {
  ...
  transform(valores: any[], parametro: string, activo: string): any {
    ...
    if(parametro.length !== 0){
      return filtrados.filter( valor => { 
        return (valor.apellido.toLocaleLowerCase() + ' ' + valor.nombre.toLocaleLowerCase()).includes(parametro) ||
               (valor.nombre.toLocaleLowerCase() + ' ' + valor.apellido.toLocaleLowerCase()).includes(parametro)
      });
    }else{
      return filtrados;
    }
